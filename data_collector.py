...
class DataCollector:
    ...
    def match_playerdata_file_name(self, match_id: int) -> Path:
        return Path(f"wrold_cup/match_{match_id}_player_stats.json")
    def fixture_file_name(self, fixture_date: date) -> Path:
        return Path(f"match_list/{fixture_date}_matches.json")

    def get_match_playerdata(self, match_id: int, force_download=False):
        ...
        file_path =  self.match_playerdata_file_name(match_id)
        if file_path.exists() and not force_download:
            ...
            return self.file_manager.save_fixture_player_stats(match_id, file_path=file_path)
        

    def get_fixtures(self, fixture_date: date = None, team=None, league=None):
        '''Gets the playerdata from a game, if already saved, gets it from local file
        force_download forces it to update the file from api'''
        ...
        file_path = self.fixture_file_name(fixture_date)
        if file_path.exists():
            print("Got data from lokal file")
            data = self.file_manager.load_json(file_path)
        else:
            print("Got data from api")
            data = self.file_manager.save_matches_by_date(fixture_date, file_path=file_path)
    ...
    def calculate_player_points(self, raw_player_stats):
        rules = self.file_manager.load_json(Path("rating_rules/example.json"))

        player_stats = self.process_player_data(raw_player_stats)
